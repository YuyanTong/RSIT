...
clear
close all
load rmse_station_all_South_central_Alaska
rmse_station_all_use=rmse_station_all_South_central_Alaska;
load combined_sd_y_South_central_Alaska
combined_sd_y_sc1=combined_sd_y_South_central_Alaska;
ydata=[zeros(1,365-length(combined_sd_y_sc1)) combined_sd_y_sc1];
rmse_station_all_predi_4=[zeros(16,365-size(rmse_station_all_use,2)) rmse_station_all_use];
...
load South_central_Alaska_earthquake_event
earthquake_matrix=South_central_Alaska_earthquake_event(1,:);
...
for i=4:length(single)
    if single(i)==1
        bbb=single(i-3:i-1);
        if sum(bbb)>0
            single(i-3:i-1)=zeros(1,3);
        end
    end
end
%正确预测地震
...
hold on
Y4=-Y2;
title('South central Alaska (1/1/2018-12/31/2018)')
hold on
...
set(gca,'xlim',datetime([2018 2019],[1 1],[1 1]),'ylim',[-10 9]);
AAAA(1)=TP;
AAAA(2)=FN ;
AAAA(3)=FP ;
AAAA(4)=TN ;
